...
public abstract class UIPage : MyComponent
{
    ...
    //refresh page 's data.
    private object m_data = "0";

    private static bool isRefresh = true;
    public object data
    {
        get
        {
    ...
    private static void _ShowPage(string pageName, UIPage pageInstance, Action callback, object pageData, bool isAsync)
    {
        ...
        //if (page.isActive == false)
        {
            ...
            if (pageData != null)
                UIPage.isRefresh = int.Parse(pageData.ToString()) >= 0;
            //is int.Parse(pageData.ToString())>=0;
